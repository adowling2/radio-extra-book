...
that can be re-derived from the element laws, KCL, and KVL. Pinpoint literature
citations appear in the derivation sections named in the last column.
...
\(f_{\mathrm{SRF}}=1/(2\pi\sqrt{L_sC})\) & Lossless self-resonance estimate of a real part; parasitics can invert its apparent type nearby. & \cref{sec:selfresonance} \\
...
\(f_{\text{peak}}\approx f_{\mathrm{SRF}}/\sqrt5\) & High-\(Q\) estimate of a coil's \(Q\) peak: \(\sqrt f\) rise against the \(C_p\) fall. & \cref{sec:selfresonance} \\
...
\(\tau=R_{\mathrm{Th}}C_{\mathrm{eq}}\) & Time constant of a one-capacitor network; fails for two stores. & \cref{sec:rthc} \\
\(\Delta f\approx1/T_{\mathrm{gate}}+f\cdot\mathrm{ppm}/10^6\) & Frequency counter: the timebase term dominates at RF. & \cref{sec:instrumentlimits} \\
...
\(y=\sum_n G(\jj n\omega_0)c_n\ee^{\jj n\omega_0 t}\) & \(G\) at each harmonic determines that harmonic's response, so one sinusoid can be analyzed at a time. & \cref{sec:fourier} \\
...
\(\mathrm{IL}=-20\log_{10}(1-Q_L/Q_U)\) & Resonator insertion loss; why duplexers need cavities. & \cref{sec:qlqu} \\
\(\tau=Q/(\pi f_0)=1/(\pi\,BW)\) & \(1/\ee\) amplitude-decay time of one isolated resonant pole pair. & \cref{sec:ringdown} \\
...
\(L=-1\): \(|L|=1,\ \angle L=-180^\circ\) & Linearized Barkhausen marginal-stability condition. & \cref{ch:feedback} \\
\(L_{\mathrm{th}}=(\partial I_C/\partial T)\theta_{JA}V_{CE}\ge1\) & Thermal runaway: positive feedback at DC. & \cref{sec:thermalrunaway} \\
\(\Delta V=2\beta V_{\mathrm{sat}}\) & Comparator hysteresis width from positive feedback. & \cref{sec:hysteresis} \\
\(Z=R\parallel sL\) & Parasitic suppressor: loss only above \(\omega=R/L\). & \cref{sec:neutralization} \\
...
\(VF=1/\sqrt{\varepsilon_r}\) & Velocity factor from the dielectric; \(0.66\) for solid PE. & \cref{ch:lines} \\
...
\(Z_0\sqrt2\) sections, \(2Z_0\) bridge & Wilkinson divider: even mode matches, odd mode isolates. & \cref{sec:wilkinson} \\
...
\(|F|=2|\sin(\beta h\sin\psi)|\) & Horizontal antenna over ground; first lobe at \(\sin\psi=\lambda/4h\). & \cref{sec:groundimage} \\
\(Z=4R_d\approx\SI{300}{\ohm}\) & Folded dipole: current doubling, squared. & \cref{sec:foldeddipole} \\
\(\mathrm{EIRP}=\mathrm{ERP}+\SI{2.15}{\decibel}\) & Same station, dBi instead of dBd. & \cref{sec:antennalength} \\
\(R(z)=R_{\mathrm{center}}/\cos^2\beta z\) & Feed-point resistance vs position; \(2\times\) at \(\lambda/8\), \(\to\infty\) at the end. & \cref{sec:feedpoint} \\
...
\(\mathrm{F/B}=10\log_{10}(p_{\rm front}/p_{\rm back})\) & Front-to-back ratio from normalized pattern power. & \cref{sec:patternmetrics} \\
\(P_r\propto\cos^2\alpha\) & Ideal polarization mismatch for two linear antennas. & \cref{sec:polarization} \\
\(G\approx\eta_{\rm ap}4\pi A/\lambda^2\) & Aperture gain; fixed-dish frequency doubling gives \SI{6}{\decibel}. & \cref{sec:aperturegain} \\
\(\jj Z_0\tan\beta\ell\) & Shorted-stub reactance. & \cref{sec:stubs} \\
\(|X|=Z_0\) at \(\ell=\lambda/8\) & The only stub length where reactance matches the line. & \cref{sec:stubs} \\
...
\addlinespace
\multicolumn{3}{@{}l}{\textbf{Modulation and receiver architecture}}\\
\(B_{\rm AM}=2f_m\) & AM bandwidth for a message limited to \(f_m\). & \cref{sec:am} \\
\(P_{\rm AM}=P_c(1+m^2/2)\) & Total conventional-AM power. & \cref{sec:am} \\
\(\beta=\Delta f/f_m\), \(B\approx2(\Delta f+f_m)\) & FM index and Carson's-rule bandwidth estimate. & \cref{sec:anglemod} \\
\(f_{\rm IF}=|f_{\rm RF}-f_{\rm LO}|\) & Mixer translation to intermediate frequency. & \cref{sec:receiverarchitecture} \\
\(f_{\rm image}=f_{\rm RF}+2f_{\rm IF}\) & High-side-injection image frequency. & \cref{sec:receiverarchitecture} \\
\addlinespace
\multicolumn{3}{@{}l}{\textbf{Devices, bias, and regulation}}\\
\(r_d\approx nV_T/I_D\) & Diode's local incremental resistance at a bias current. & \cref{sec:devicebias} \\
\(g_m\approx I_C/V_T\) & BJT transconductance near its operating point. & \cref{sec:devicebias} \\
\(A_v\approx-R_C/(r_e+R_E)\) & Common-emitter gain with unbypassed emitter degeneration. & \cref{sec:loadlines} \\
\(\Delta V\approx I_{\rm load}/(f_{\rm ripple}C)\) & Capacitor-input supply ripple estimate. & \cref{sec:regulation} \\
\addlinespace
...
\(\mathrm{IIP3}\approx P_{\SI{1}{\decibel}}+\SI{9.6}{\decibel}\) & Cubic relation using conventional single-tone \(P_{\SI{1}{\decibel}}\) and two-tone IIP3. & \cref{sec:powerseries} \\
...
\(\mathcal{L}\propto 1/(Q_L^2\Delta f^2)\) & Ideal resonator-filtered, far-from-flicker-corner phase-noise scaling; doubling \(Q_L\) buys \SI{6}{\decibel} within that model. & \cref{sec:phasenoise} \\
