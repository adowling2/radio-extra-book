...
for b, c in zip(betas, colors):
    ...
    axp.plot([sigma, sigma], [wd, -wd], "x", color=c, ms=10, mew=2.1,
             label=rf"$\beta={b:g}$")
    axp.annotate(rf"$\beta={b:g}$", xy=(sigma, wd), xytext=(7, 0),
                 textcoords="offset points", color="0.22", fontsize=6.7, va="center")
...
axp.annotate(r"$\sigma=-\zeta\omega_0$ fixed", xy=(sigma, 2.75),
             xytext=(-1.35, 2.85), color="0.35", fontsize=7.5, ha="left")
...
for b, c, ls in zip(betas, colors, styles):
    ...
    axs.plot(t, y, color=c, ls=ls, lw=1.6, label=rf"$\zeta_{{cl}}={zcl:.2f}$")
