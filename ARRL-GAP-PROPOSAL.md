...
Score: **58 done, 0 open** (58 items). **The ledger is complete.** Nothing has been silently dropped.
...
**Tier D — housekeeping (four items).** *Ch 21 rebuilt.* It was the weakest chapter: four isolated examples with component values unrelated to anything else in the book and zero cross-references, in the chapter meant to demonstrate the thesis. Of the two options the plan offered — pure map, or rebuild on the running values — I took the rebuild, since it fixes the defect without discarding content. It now works **one circuit through seven questions** (Ch 13's own `L = 10 µH, C = 220 pF, R = 5 Ω`) and **one antenna through three** (Ch 16's 35 Ω dipole), every answer cross-referenced to where it was derived, closing with a `controlsbox` on why one object answers seven questions. It also gained its first callout boxes. *Figure provenance:* rather than 27 hand-written caption clauses, a front-matter note states the rule — every computed figure is generated by the identically-named script in `figures/src/`, so any curve can be regenerated or re-plotted, and nothing is traced or copied. Lower churn and more maintainable than per-caption text. *Ch 22's two registers:* the mixture was real but not accidental — some groups deserve question-by-question arithmetic, others collapse onto one idea. The chapter now declares both registers up front and tags every section as *Question by question* or *Group synthesis*, so the pattern is intentional and navigable. *Appendix B:* kept, because a one-page lookup earns its place even when the reasoning lives in Ch 7 — but it was made genuinely non-duplicative by adding a third table of the **compound and referenced units** the later chapters introduce one at a time and never gather (VA, VAR, dBW/dBm, dBm/Hz, dBi/dBd, nepers with the 8.686 conversion, ppm, K/W, S/m, H/m, S units), plus a `mistakebox` on the two pairs that cause trouble: VA/VAR are not watts, and a bare dB is not a level.
