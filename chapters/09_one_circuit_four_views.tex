...
state-space models, transfer functions, and frequency responses), and the earlier
Part~III chapters supplied the pieces: the modeling paradigm and components
(\cref{ch:foundations}), resistive networks (\cref{ch:resistive}), and the AC
steady-state view---impedance and phasors---of \cref{ch:ac}. This short chapter
...
views are complementary rather than competing: each makes a different feature of
...
The generic systems of \cref{ch:linsys} have no schematic, so there the four
views are the differential equation, state-space model, transfer function, and
frequency response. A \emph{circuit} adds the physical network as the concrete
starting point and reads the transfer function together with its frequency
response---the same machinery, entered through the wiring diagram.
