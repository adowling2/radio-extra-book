...
\label{ch:exammap}
This chapter maps the derivations of Parts~II--IV onto exam-style questions. It
works one circuit (\cref{sec:onecircuit}) and one antenna (\cref{sec:oneantenna})
through the exam's questions, using values unchanged from the chapters that derived
them. Every number can therefore be traced. \Cref{ch:practice} reasons through
individual pool items; \cref{ch:crossproblems} requires several chapters at once.
...
Isolated examples teach the arithmetic and hide the point. Consider the running
series RLC circuit from \cref{ch:rlc}, with
\[
L=\SI{10}{\micro\henry},
\qquad
C=\SI{220}{\pico\farad},
...
At \(f_0\), both reactances are \SI{213}{\ohm} and cancel exactly. Checking that
\(X_L=X_C\) confirms resonance.
...
with \(\tau=1/\alpha=\SI{4}{\micro\second}\). The bandwidth relation
\(\tau=1/\pi BW\) from \cref{sec:ringdown} gives the same value. The two routes
agree because they describe the same pole pair.
...
\begin{controlsbox}
\textbf{One circuit, seven questions.} Reactance, resonance, \(Q\), bandwidth,
magnification, pole location, and ring-down are seven readings of one second-order
transfer function. Learn the object, not seven isolated facts. If a question resists,
identify whether it asks for the pole pair in frequency, time, or the complex plane;
\cref{ch:fourviews} connects those views.
\end{controlsbox}
For additional treatments of the same resonance, bandwidth, and magnification
relationships, see \cite[pp.~59--67]{white2004highfrequency} and
...
\emph{radiated}, which is \cref{sec:antennaeff}'s \(\eta\) and cannot be inferred
from shack-side SWR or reflection alone.
The equivalent reflection, return-loss, SWR, and delivered-power descriptions are
developed further in \cite[pp.~8--14]{free2022rfmicrowave}.
