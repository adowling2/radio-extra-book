...
\emph{backward} from resistors---and it is worth seeing why rather than
...
\begin{physicalbox}
\textbf{Why capacitors look ``backward.''} Capacitance is charge stored per volt,
...
\end{tabularx}
\end{center}
The same rules follow in sinusoidal steady state by adding series impedances or
parallel admittances \cite[pp.~28--32]{white2004highfrequency}.
...
\draw (6,0) to[I,l_=\(I_N\)] (6,2) -- (9,2) to[short,-o] (10,2);
\draw (6,0) -- (9,0) to[short,-o] (10,0);
\draw (8,2) to[R,l=\(R_N\)] (8,0);
\end{circuitikz}
...
For a purely resistive Thevenin source, maximum load power occurs when
\[
R_L=R_{Th},
\]
For complex impedances, the maximum-real-power condition is
\(Z_L=Z_S^{*}\) \cite[pp.~39--40]{white2004highfrequency}. A separate
transmission-line condition minimizes reflection: the load equals the line's
characteristic impedance. In a conventional resistive \(\SI{50}{\ohm}\) system
these conditions are coordinated, but source conjugate matching and line/load
matching remain distinct concepts.

\begin{exambox}
\(\SI{50}{\ohm}\) is a design convention. Maximum available-power transfer
concerns conjugate source/load impedances; minimum line reflection concerns
equality between load and characteristic impedance.
\end{exambox}
