...
\label{ch:noise}
A receiver is judged by the weakest signal it can hear and the strongest it can
tolerate. The noise floor follows from thermal physics and \cref{ch:resistive}'s
maximum-power-transfer condition; strong-signal limits follow from the nonlinearity
of \cref{sec:mixers}.

The lower limit is \emph{noise}: how much the receiver adds and where it adds it.
The upper limit is \emph{nonlinearity}: how the front end fails when overdriven.
Their gap is the \textbf{dynamic range}; one power series yields its several measures.
...
\boxed{\;P_n=kTB\;}
\]
This available-noise-power result is developed in
\cite[pp.~257--260]{weber2001microwave}.
The resistance cancels: available noise power depends on temperature and bandwidth,
not on the resistor. The factor of \(4\) in the Johnson formula yields this result,
showing that available power, rather than open-circuit voltage, is the relevant
quantity.
...
\]
This is the conventional reference for receiver noise calculations
\cite[p.~48]{egan2003practical}. It is the available noise density delivered by a
matched source at \(T_0=\SI{290}{\kelvin}\), not an absolute lower bound independent
of source temperature; a colder source has lower thermal noise.
...
Receiver sensitivity is quoted both ways, so the bridge is worth having. If \(V\)
is the RMS voltage \emph{across a matched \SI{50}{\ohm} load}, then
\(P=V_{\RMS}^2/R\), so
\[
...
\cite[pp.~262--263]{weber2001microwave}.
...
\]
The cascade relation is developed in
...
\item[A loss ahead of the receiver costs its full value.] At the reference
temperature, a passive attenuator or feed line with linear loss \(L\) has \(F=L\)
and \(G=1/L\), so \(n\) decibels of line loss adds \(n\) decibels to system noise
figure. There is no gain ahead of it to discount it.
...
For the convention used here, the \textbf{minimum discernible signal} (MDS) is the
input level equal to the receiver's integrated noise, corresponding to
\SI{0}{\decibel} input SNR:
\[
\mathrm{MDS}=-174+10\log_{10}B+\mathrm{NF}_{\dB}.
\]
A \SI{3}{\decibel} increase in measured total output power is one practical test:
equal signal and noise powers double the noise-only reading. Instrument bandwidth,
detector, and the required output criterion must be reported because other sensitivity
specifications use different conventions.
...
---\SI{7}{\decibel} worse, purely from bandwidth. Improving noise figure from
\SI{8}{\decibel} to \SI{3}{\decibel} buys \SI{5}{\decibel} under the internal-noise
model. On portions of HF at many sites, external atmospheric or man-made noise can
dominate, but the outcome depends on frequency, location, time, and antenna.
\end{workedbox}
...
The polynomial model and the one-, two-, and three-to-one logarithmic slopes are
developed in \cite[pp.~89--103]{egan2003practical}.

\subsection{Gain Compression}
\label{sec:compression}
The cubic term does something else too. For the two equal tones used above,
collecting the components at \(\omega_1\) itself---the cube of a sum contributes
back to the fundamental---gives the effective gain
\[
a_1+\tfrac94a_3A^2
\]
For a single tone, the corresponding coefficient is
\[
a_1+\tfrac34a_3A^2.
\]
In the usual compressive region \(a_3\) has the \emph{opposite} sign to \(a_1\), so
the gain falls as drive increases. That single term is gain compression, blocking, and
desensitization---three names for one coefficient. The standard
\textbf{\(\SI{1}{\decibel}\) compression point} is a \emph{single-tone}
measurement: it is where
\[
a_1+\tfrac34a_3A^2=10^{-1/20}a_1\approx0.891\,a_1.
\]
...
before reaching it, so IIP3 is best interpreted as a linearity coefficient wearing the units of
power. Second, the standard input-referred single-tone
\(P_{\SI{1}{\decibel}}\) and input-referred two-tone IIP3 depend on the same
coefficient ratio in a pure cubic model. Under those conventions,
\[
\mathrm{IIP3}\approx P_{\SI{1}{\decibel}}+\SI{9.6}{\decibel}
\]
The cubic-model spacing is derived in
\cite[pp.~102--103]{egan2003practical}.
This is a useful consistency check, not a universal spacing: higher-order terms and
device-specific compression change it.
...
\]
This two-thirds result is the standard third-order, noise-limited geometry
\cite[pp.~137--139]{egan2003practical}. Data sheets and test reports may label
closely related bandwidth- and threshold-dependent quantities as IMD dynamic range,
instantaneous dynamic range, or spurious-free dynamic range, so compare definitions
...
3L-L=2L\ \si{\decibel} .
\]
Ten decibels of attenuation therefore buys \SI{20}{\decibel} of relief from IM3
generated in stages after the attenuator. The cost, by \cref{sec:friis}, is
\(L\) decibels of receiver noise figure. That cost has little effect on system SNR
only when external noise exceeds receiver-added noise by a large margin; it is not
literally free, and the condition is frequency-, site-, and antenna-dependent.
\end{workedbox}

\section{Phase Noise and Reciprocal Mixing}
\label{sec:phasenoise}
Phase noise is another mechanism that raises the noise floor. The model below isolates
the resonator-filtered part of an oscillator's spectrum; it does not describe every
...
\]
The expansion separates a pure carrier from a term carrying phase noise in quadrature.
A jittering oscillator therefore has a carrier with \textbf{skirts}. Their density at
...
indistinguishable from noise. If the quoted single-sideband phase-noise density is
approximately constant across the receiver noise bandwidth \(B\), the resulting
noise power is estimated by
\[
P_{\text{noise}}=P_{\text{interferer}}+\mathcal{L}(\Delta f)+10\log_{10}B ,
\]
The reciprocal-mixing mechanism and its phase-noise dependence are discussed in
\cite[pp.~245--250]{egan2003practical}.
with \(\mathcal{L}\) the phase-noise density. This is \textbf{reciprocal mixing}, and
its signature is distinctive: the noise floor \emph{rises as you tune toward} a strong
...
An idealized, far-from-flicker-corner Leeson-type scaling connects one part of the
skirt to a pole. Hold carrier power and the relevant noise sources fixed, and model the
tank as filtering the active device's noise. By \cref{ch:rlc}, a resonator of loaded
quality factor \(Q_L\) is a band-pass whose half-bandwidth is \(\omega_0/2Q_L\).
Above that corner, this model rolls off at \(-\SI{20}{\decibel}\) per decade:
\[
\mathcal{L}(\Delta f)\ \propto\ \frac{1}{Q_L^2}\cdot\frac{1}{\Delta f^{2}} .
\]
Within this model, the \(-\SI{20}{\decibel}\)/decade slope is the single-pole
roll-off of \cref{ch:splane}, appearing as the shape of a noise skirt. The
\(1/Q_L^2\) factor means that doubling the loaded \(Q\) reduces the predicted
phase-noise density by \SI{6}{\decibel}. This quantifies the tank-loading trade-off
of \cref{ch:crossproblems}. Actual phase-noise spectra also depend on carrier power,
active-device and resonator noise, flicker noise, and loop architecture; those
dependencies can change both the level and the offset-frequency regions. The offset
regions and reciprocal-mixing consequence are discussed in
\cite[pp.~245--250]{egan2003practical}.

\begin{controlsbox}
\textbf{Tank poles shape the resonator-filtered part of the spectrum.} In the ideal
linear model, a pole exactly on the \(\jj\omega\) axis gives an infinitely thin spectral
line. Real loss and noise broaden that line into a skirt. Within the stated
resonator-filtered model, high \(Q_L\) produces a narrower skirt and less reciprocal
mixing. Carrier power, device and resonator noise, flicker noise, and loop architecture
set the remaining phase-noise behavior.
\end{controlsbox}
...
\]
Equivalent noise bandwidth is defined and applied in
\cite[p.~49]{egan2003practical}.
...
For the simple rectangular-spectrum argument used here, collected signal power stops
increasing once the passband covers the occupied signal bandwidth, while white-noise
power continues to grow with equivalent noise bandwidth:
\[
\mathrm{SNR}(B)=\frac{S(B)}{N_0B}
\]
This makes \(B\approx B_{\text{signal}}\) a useful receiver-filter heuristic. More
generally, optimum detection is a matched-filter problem, so the best response depends
on signal shape and detection criterion. A filter that is too wide admits unnecessary
noise; one that is too narrow distorts or truncates the signal.
