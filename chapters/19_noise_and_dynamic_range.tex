...
power. Second, the standard single-tone \(P_{\SI{1}{\decibel}}\) and two-tone IIP3
are both set by the same ratio \(|a_1/a_3|\), so they are not independent. Dividing
the two expressions gives
\[
\mathrm{IIP3}\approx P_{\SI{1}{\decibel}}+\SI{9.6}{\decibel}
\]
for a purely cubic nonlinearity, with those stated measurement conventions. A useful
sanity check on a datasheet: if the two numbers differ by wildly more or less than
\SI{10}{\decibel}, something beyond the cubic term is at work.
...
dependencies can change both the level and the offset-frequency regions.
