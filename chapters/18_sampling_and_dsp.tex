...
\]
Thus a symmetric FIR filter delays every component equally and introduces no
waveform distortion from phase, regardless of magnitude response. This proves the
claim of \cref{ch:filters}; symmetry, not poles, is sufficient.
...
\begin{controlsbox}
\textbf{FIR versus IIR.} An FIR filter is a delay cascade with no loop. It cannot
ring and can have exactly linear phase, but needs many taps for selectivity. An IIR
filter is a feedback loop, so it gains efficiency and poles, along with instability
when a coefficient places a pole outside the unit circle. This is the analog
trade-off in arithmetic.
\end{controlsbox}
