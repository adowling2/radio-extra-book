...
though it does not add a single bit. This is why converters are run far above the
Nyquist rate and why an SDR's in-band performance can exceed its raw converter
...
sustain ringing, and with symmetric coefficients it can have exactly linear phase,
but it may need many taps for selectivity. An IIR filter is a feedback loop, so it
gains efficiency and poles, along with instability when a coefficient places a pole
outside the unit circle. This is the analog trade-off in arithmetic.
\end{controlsbox}
