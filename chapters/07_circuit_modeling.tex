...
climbs, roughly as
\[
R_{\mathrm{AC}}\propto\sqrt{f} .
\]
Three consequences the rest of the book uses: an inductor's \(Q=\omega_0L/R\) does
\emph{not} improve indefinitely with frequency, because \(R\) is rising too;
RF conductors are stranded, tubular, or silver-plated, since only the surface
matters; and a transmission line's attenuation inherits the same \(\sqrt f\) law
(\cref{sec:lineloss}).
