...
The carrier/sideband expansion, twice-audio occupied bandwidth, and one-third total
sideband power at 100 percent sinusoidal modulation are treated in
\cite[pp.~185--192]{carr2002components}.
...
Suppressing the carrier and one sideband produces the corresponding bandwidth and
power economy \cite[pp.~191--192]{carr2002components}.
...
apart.
The fixed-IF superheterodyne arrangement and the need to reject image responses before
they reach the IF chain are discussed in
\cite[pp.~123--125]{logvinov2022receivers}.
...
the noise bandwidth (\cref{sec:noisebw}). Direct-conversion receivers translate RF
into analog in-phase and quadrature baseband; software-defined architectures digitize
at different points and perform some or most subsequent filtering numerically. The
precise boundary is architecture-dependent, while every sampling stage still obeys
the aliasing constraints of \cref{ch:dsp}
