...
\cref{sec:antennaeff}.
These pattern, beamwidth, directivity, gain, and efficiency distinctions are summarized
in \cite[pp.~359--363]{free2022rfmicrowave}.
...
Orthogonal components and quadrature excitation are also used to generate circular
polarization in practical antenna structures
\cite[pp.~397--398]{free2022rfmicrowave}.

\section{Why Apertures Gain \SI{6}{\decibel} When Frequency Doubles}
\label{sec:aperturegain}
A reflector or dish has effective aperture \(A_{\rm eff}=\eta_{\rm ap}A\), so its gain is
\[
\boxed{\quad G=\frac{4\pi A_{\rm eff}}{\lambda^2}
=\eta_{\rm ap}\frac{4\pi A}{\lambda^2}.\quad}
\]
Aperture efficiency accounts for illumination taper, blockage, surface error, and
spillover \cite[pp.~407--409]{free2022rfmicrowave}. For fixed physical area
\emph{and unchanged aperture efficiency}, \(G\propto f^2\); doubling frequency then
adds \SI{6.02}{\decibel}. In a real system, feed pattern, conductor loss, surface
accuracy, and blockage can change with frequency, so the \SI{6}{\decibel} result is
conditional. Conversely, keeping gain while doubling wavelength requires four times
the aperture area if aperture efficiency is unchanged.
...
form.  It is the antenna analog of replacing a distributed line by many small
...
\item[Put reactance where current is useful.] A center or upper loading coil lies
...
add forward and partly cancel behind. This is the interference calculation of
\cref{sec:arrayfactor}, now with currents set by coupling rather than prescribed by
two feed lines.
This coupled parasitic-element description agrees with the Yagi--Uda treatment in
\cite[pp.~378--385]{free2022rfmicrowave}.
\end{description}
...
Now keep a dish's physical aperture fixed and move from
\SI{1.2}{\giga\hertz} to \SI{2.4}{\giga\hertz}. If aperture efficiency remains
unchanged and the feed and reflector remain usable, the wavelength halves and the
formula predicts \(G_2/G_1=4\), or \SI{6}{\decibel} more gain
\cite[pp.~407--409]{free2022rfmicrowave}. The gain change comes from the aperture
becoming electrically larger, not from extra transmitter power.
\end{workedbox}
...
Gain and efficiency are separate from pattern normalization and impedance match
\cite[pp.~359--363]{free2022rfmicrowave}.
\end{mistakebox}
