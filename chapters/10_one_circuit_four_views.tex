...
Four rows look like a short list for a whole exam syllabus, and it is fair to ask
what happens to the circuits that do not fit---a five-pole filter, a three-stage
amplifier, a feed line. The answer is the factoring theorem of
\cref{ch:highorder}: each of those is a \emph{cascade} of the rows above, so the
table does not need more rows. Part~IV spends its four chapters cashing that in.
