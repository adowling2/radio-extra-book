...
The sections below come in two deliberate registers, because the pool's groups are
not all alike. Where the arithmetic is the difficulty, a section walks
\textbf{question by question}, each with its own heading and identifier. Where a whole
group collapses onto a single idea---so that answering any of its questions is the
same act---the section instead gives a \textbf{group synthesis} in prose and names the
chapter that supplies the idea. Each section says which it is, so a reader looking for
worked arithmetic can skip to the ones that have it.
...
product---not whose average---returns \(f_0\).
...
\]
so \(Z\approx300+\jj396\,\si{\ohm}\)---first quadrant (resistance positive, reactance
...
wavelengths, \(\lambda=VF\,c/f\), not in meters---so the same conductor is
...
down, and both are derived in \cref{sec:selfresonance}: every real component has a
self-resonant frequency, above which it behaves as its \emph{opposite} type, and skin
effect drives its AC resistance up as \(\sqrt f\)---the latter from \(\delta=\sqrt{2/\omega\mu\sigma}\),
so the exponent is derived and not quoted. For the exam the two consequences
worth carrying are that a bypass capacitor or an RF choke simply stops working above
its own \(f_{\mathrm{SRF}}\), and that a coil's \(Q\) peaks near
\(0.45f_{\mathrm{SRF}}\) and falls above it. Be careful with the reason for that
second one: skin effect alone would still let \(Q\propto\sqrt f\) keep \emph{rising},
because \(\omega L\) grows faster than \(\sqrt f\) does. What pulls \(Q\) back down is
the inter-turn capacitance, which drives \(Q_{\mathrm{eff}}\) to zero at
\(f_{\mathrm{SRF}}\). \Cref{ch:measurement} runs the same
...
or a quarter-wave \(Z_t=\sqrt{Z_0Z_L}\) from \cref{ch:lines}.
