...
\label{ch:practice}
This chapter applies the theory to the Amateur Extra question pool
(Element~4, valid 2024--2028). Each problem gives its pool identifier and reasons
to an answer through conservation, the appropriate view, and the s-plane picture
rather than memorizing a letter. Attempt each before reading the reasoning.
Correct-choice letters follow the current pool.

The pool groups require two forms of practice. Where arithmetic is the challenge,
the chapter proceeds \textbf{question by question}. Where one idea resolves a whole
group, it gives a \textbf{group synthesis} and names the chapter that supplies the
idea. Each section identifies its form.
...
poles sit at \(-\alpha\pm\jj\omega_d\) with \(\omega_d\approx\omega_0\) because the
circuit is high-\(Q\).
...
leaving \(Z\approx R\), the circuit resistance. This is why series
...
\emph{circulating inside} the tank is at a maximum (E5A06~B). Energy moves between
\(L\) and \(C\) while the source only makes up the losses. This is the dual of the
series case.
...
\]
This is an equality, not an estimate (\cref{sec:halfpower}). In pole terms the
half-power bandwidth equals twice the pole's distance from the \(\jj\omega\) axis;
high \(Q\) presses the poles against the axis and narrows the peak. If a question
asks for the band edges rather than the width, remember they straddle \(f_0\)
geometrically: here \SI{7.076}{\mega\hertz} and \SI{7.124}{\mega\hertz}, whose
product, not their average, returns \(f_0\).
...
reactance. Inverting a purely imaginary \(Z\) takes its reciprocal and keeps it on
the imaginary axis.
...
negative (capacitive) reactance. The sign convention distinguishes it from an
inductor, which would be \(0+\jj100\).
...
\]
so \(Z\approx300+\jj396\,\si{\ohm}\), in the first quadrant: resistance and
reactance are both positive. The matching figure point is up and to the right.

\section{RF Effects in Components and Circuits (E5D)}
\emph{Group synthesis.}
This group has three distinct halves, and the book is strong on two of them.

\emph{Real and reactive power} is \cref{ch:ac} directly: \(S=VI^{*}=P+\jj Q\), where
\(P\) (watts) is dissipated and \(Q\) (VAR) merely shuttles in and out of storage each
cycle, so a purely reactive load consumes no average power. Apparent power \(|S|\) in
volt-amperes exceeds \(P\) whenever there is reactance.

\emph{Electrical length} is \cref{ch:lines}: what matters at RF is length measured in
wavelengths, \(\lambda=VF\,c/f\), not in meters. The same conductor can be
electrically short at HF and a significant fraction of a wavelength at VHF.

\emph{Parasitics and skin effect} are where the lumped-element idealization breaks
down, and both are derived in \cref{sec:selfresonance}: real components have
self-resonance and parasitics that eventually overturn their intended lumped behavior,
while skin effect drives AC resistance up as \(\sqrt f\) in the stated model. The
skin-depth relation \(\delta=\sqrt{2/\omega\mu\sigma}\) derives that exponent. For
the exam, the useful consequences are simple: a bypass capacitor or RF choke has a
finite useful range, and a coil's \(Q\) eventually falls as inter-turn
capacitance matters. In the high-\(Q\) simplified model, its peak is near
\(0.45f_{\mathrm{SRF}}\); real loss and additional parasitics shift that value. Skin
effect alone would still let \(Q\propto\sqrt f\) keep \emph{rising}, because
\(\omega L\) grows faster than \(\sqrt f\) does. The inter-turn capacitance pulls the
model back down near self-resonance. \Cref{ch:measurement} runs the same
effect in reverse when it computes a probe ground lead's resonance, and
\cref{sec:lineloss} inherits the \(\sqrt f\) law as feed-line attenuation.
...
matching question has two parts: cancel reactance, then transform resistance. Use
the L-network \(Q=\sqrt{R_{\text{hi}}/R_{\text{lo}}-1}\) or the quarter-wave
\(Z_t=\sqrt{Z_0Z_L}\) from \cref{ch:lines}.
...
reduce to the Barkhausen condition: unity loop-gain magnitude and net zero phase,
or a pole pair placed on the \(\jj\omega\) axis
...
dissipates it, and which of the four views (physical network, differential
equation, state-space, or frequency response) answers the question most directly. For anything
