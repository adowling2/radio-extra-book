...
model with \(n\) independent states has an \(n\times n\) state matrix, and its
characteristic polynomial \(\det(sI-A)\) has degree \(n\):
\[
\dot{\state}=A\state+Bu,
\qquad
\state\in\mathbb{R}^n .
\]
Thus \(A\) has \(n\) eigenvalues counting algebraic multiplicity. A selected
input--output transfer function displays all \(n\) as poles only when the
realization is minimal; uncontrollable or unobservable modes can be hidden
...
\cite[pp.~32--35, 42--44]{leigh2012control}.
...
\angle G=\angle G_1+\angle G_2 \;}
\]
\cite[pp.~48--61]{leigh2012control}
...
\omega=\sqrt3\,\omega_0 . \;}
\]
This result is exact for the three identical, stable first-order lags and ordinary
negative-feedback convention stated above.
...
\]
\cite[pp.~139--140]{leigh2012control}
...
amplitude. A real delay can therefore supply enough additional phase for a loop to
reach a phase crossover;
...
Push the order up without bound and something qualitatively new appears. A cascade
of \(n\) identical lags, each with time constant \(T/n\), tends to a pure delay:
\[
\lim_{n\to\infty}\left(1+\frac{sT}{n}\right)^{-n}=\ee^{-sT}.
\]
This is a useful bridge to a \emph{matched, one-way} propagation path, but not a
complete transmission-line model. A physical line is distributed; characteristic
impedance, loss, dispersion, termination, and reflections determine its full
response and any standing-wave resonances.
