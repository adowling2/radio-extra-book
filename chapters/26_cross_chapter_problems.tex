...
\label{ch:crossproblems}
Each problem chains results from \emph{several} chapters, as real analysis does.
Attempt it before reading the solution, and note the chapters you needed.
...
\]
One octave below the corner costs about \SI{7}{\decibel}. A coupling corner is
therefore placed well below the lowest wanted frequency, not at it.
...
\]
The bandwidth specification fixes \(Q\) through \(BW=f_0/Q\) (\cref{sec:halfpower}):
\[
...
need high-\(Q\) components because the pole must sit that close to the
\(\jj\omega\) axis.
...
\subsection*{Solution}
(a) The loss is physically the coil's series resistance
\(r=\omega_0L/Q=98.1/200\approx\SI{0.49}{\ohm}\), so getting a shunt resistance out of
it is the series-to-parallel transformation of \cref{sec:seriesparallel}:
\[
R_p=(1+Q^2)\,r\approx Q^2r=Q_p\,\omega_0L=(200)(98.1)\approx\SI{19.6}{\kilo\ohm},
\]
and that is the purely resistive impedance at resonance. It is a \emph{maximum}, the
mirror of the series trap above. Note the leverage: half an ohm of coil
loss becomes twenty kilohms of tank impedance, because the transformation multiplies
by \(Q^2\). The \(\approx\) costs \SI{25}{ppm} at \(Q=200\); at low \(Q\) use
\((1+Q^2)\).
...
\]
so \(BW=f_0/Q\approx\SI{175}{\kilo\hertz}\). Loading cut \(Q\) by a factor of
...
oscillator's spectrum smears, producing worse phase noise. Couple the next stage
loosely to trade signal amplitude for spectral purity.
...
\]
Tuning out the reactance did most of the work, but \(\SI{35}{\ohm}\ne\SI{50}{\ohm}\).
This is the two-part job a matching network
...
\]
so the DC reading is only \(0.5\%\) low, which is acceptable here.
...
shock-excites it. A one-inch spring tip shortens the ground path and moves the
resonance above \SI{350}{\mega\hertz}.
...
answer because the asymptotic estimate is reliable this far out.) Seven reactive
elements is a lot, so real transmitter filters often add \emph{zeros}: an
...
\]
The resistance ratio fixes \(Q\); it is not a free choice. A loaded
\(Q\) of \(5.4\) means a fractional bandwidth of roughly \(1/Q\approx18\%\)
(\cref{ch:rlc}), so this match holds over a useful range but must be retuned across
