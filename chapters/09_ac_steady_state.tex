...
\label{sec:pep}
Complex power settles how much power a load consumes at one instant in the steady
state. Radio work needs two further readings of the same quantity, because a
transmitter's output is not a steady sinusoid---its amplitude moves with the
modulation.

\emph{Average power in a resistance.} Because RMS was defined precisely to make
this work (\cref{sec:rms})---and because \cref{sec:instpower} showed the average is
the constant term of \(p(t)\)---the DC formulas carry over with no correction:
\[
P=V_{\RMS}I_{\RMS}=\frac{V_{\RMS}^2}{R}=I_{\RMS}^2R .
\]
If a voltage is quoted as a peak or peak-to-peak value---which is what an
oscilloscope reads---convert first:
\[
V_{\RMS}=\frac{V_p}{\sqrt2}=\frac{V_{pp}}{2\sqrt2},
\qquad\text{so}\qquad
P=\frac{V_{pp}^2}{8R} .
\]
That last form is worth keeping, because a scope across a known load is the usual
way RF power is actually measured, and \(8R\) is easier to remember than the chain
...
\emph{average} over a cycle, just the worst cycle. Two limits make it concrete. For
an unmodulated carrier the envelope is flat, so every cycle is the largest one and
\(\mathrm{PEP}\) equals the average power exactly: the ratio is \(1.00\). For a
modulated signal the envelope varies, so PEP exceeds the average power, and by how
much depends entirely on the modulation---which is why transmitter limits are
written in PEP. It is the one number that does not depend on what you are
transmitting.
...
\SI{100}{\watt} PEP on single sideband while its heat sink never sees
\SI{100}{\watt}.
\end{workedbox}
