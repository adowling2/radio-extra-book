...
\]
In pole terms the half-power bandwidth equals twice the pole's distance from the
...
\emph{Parasitics and skin effect} are where the lumped-element idealization of
\cref{ch:foundations} breaks down. At RF a resistor has inductance, an inductor has
inter-winding capacitance, and a capacitor has lead inductance, so every real
component has a self-resonant frequency above which it behaves as its \emph{opposite}
type---the effect \cref{ch:measurement} exploits in reverse when it computes a probe
ground lead's resonance. \textbf{Skin effect} is the related loss mechanism: at high
frequency, induced eddy currents force conduction into a thin outer layer of the
conductor, so the effective cross-section shrinks and the AC resistance rises roughly
as \(\sqrt f\). This is why RF conductors are silver-plated or stranded rather than
solid, and why a coil's \(Q\) falls at the top of its range. It is the same
``modeling-assumption'' checklist as \cref{ch:measurement}: ideal element, lumped, no
parasitics.
