...
\midrule
\endhead
Reactance & \(X\) & \si{\ohm} & Imaginary part of impedance; \(X_L=\omega L\), \(X_C=1/(\omega C)\). \\
Impedance & \(Z\) & \si{\ohm} & \(Z=R+\jj X\), voltage-to-current phasor ratio. \\
Admittance & \(Y\) & \si{\siemens} & \(Y=1/Z=G+\jj B\). \\
Susceptance & \(B\) & \si{\siemens} & Imaginary part of admittance. \\
Real / reactive power & \(P,\ Q\) & \si{\watt},\ \(\mathrm{VAR}\) & \(S=P+\jj Q\); \(Q\) here is reactive power. \\
Time constant & \(\tau\) & \si{\second} & First-order clock, \(RC\) or \(L/R\). \\
Resonant frequency & \(\omega_0,\ f_0\) & \si{\radian\per\second},\ \si{\hertz} & \(\omega_0=1/\sqrt{LC}\). \\
Damped frequency & \(\omega_d\) & \si{\radian\per\second} & \(\omega_d=\omega_0\sqrt{1-\zeta^2}\). \\
Decay rate & \(\alpha\) & \si{\per\second} & Envelope decay; \(\alpha=R/2L\) (series RLC). \\
Damping ratio & \(\zeta\) & --- & \(\zeta=\alpha/\omega_0=1/(2Q)\). \\
Quality factor & \(Q\) & --- & Sharpness of resonance; \(Q=\omega_0/BW\). \\
Bandwidth & \(BW\) & \si{\hertz} & Half-power width, \(\approx f_0/Q\). \\
Complex frequency & \(s\) & \si{\per\second} & Laplace variable \(s=\sigma+\jj\omega\). \\
Forward gain & \(A\) & --- & Amplifier open-loop gain. \\
Feedback fraction & \(\beta\) & --- & Fraction of output fed back. \\
Loop gain & \(L\) & --- & \(L=A\beta\); \(L=-1\) gives oscillation. \\
Characteristic impedance & \(Z_0\) & \si{\ohm} & Line's surge impedance (often \SI{50}{\ohm}). \\
Reflection coefficient & \(\Gamma\) & --- & \(\Gamma=(Z_L-Z_0)/(Z_L+Z_0)\). \\
Standing-wave ratio & \(\mathrm{SWR}\) & --- & \((1+|\Gamma|)/(1-|\Gamma|)\). \\
Velocity factor & \(VF\) & --- & \(v_p/c\) on the line. \\
Wavelength & \(\lambda\) & \si{\meter} & \(\lambda=v_p/f\). \\
Phase constant & \(\beta\) & \si{\radian\per\meter} & \(\beta=2\pi/\lambda\) (line phase shift per length). \\
Line length & \(\ell\) & \si{\meter} & Physical length of a line section. \\
Turns ratio & \(N_p/N_s\) & --- & Transformer primary-to-secondary turns. \\
Stored energy & \(\mathcal{H}\) & \si{\joule} & Total energy in \(L\) and \(C\). \\
\bottomrule
\end{longtable}
