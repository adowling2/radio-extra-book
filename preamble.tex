...
% --- Float placement -------------------------------------------------------
% LaTeX's defaults are tuned for text-heavy documents and push figures onto
% pages of their own: a float page need only be half full, and a page carrying
% a float must still be 20 % text. In a figure-dense book that strands plots
% two pages from the prose that discusses them. These values let a figure share
% a page with text, and require a float page to be nearly full before LaTeX is
% allowed to make one.
...
\setcounter{topnumber}{3}                % was 2
\setcounter{bottomnumber}{2}             % was 1
\setcounter{totalnumber}{4}              % was 3

\pgfplotsset{compat=1.18}
\usetikzlibrary{arrows.meta,calc,positioning}
\graphicspath{{figures/}}
\sisetup{per-mode=symbol,detect-all,retain-explicit-plus}
% Feed-line loss is quoted per 100 feet by every manufacturer, so the unit is
% needed even though the rest of the book is metric.
...
% Callout boxes are color-coded by type: each has its own accent frame and a
% faint matching tint, with a colored title bar for quick visual scanning.
