...
(216 pages, 23 chapters, 35 figures, 77 section labels, 65 glossary entries). One
...
- ✅ **Six figures added** (29 → 35), each computed from material the book already
  derives, each generated by a script of the same name in `figures/src/`:
  - `halfpower_edges` — the half-power bandwidth drawn twice: as a width on a *log*
    frequency axis, where `f₀ = √(f₁f₂)` becomes visible symmetry, and as `2α` in the
    s-plane. The clearest single picture of the frequency-domain ↔ pole-location bridge.
  - `s21_is_bode` — a VNA's two traces for a band-pass, every feature labelled by the
  ...
  - `conduction_angle` — the truncated-cosine waveforms and the efficiency curve.

  *Correction:* an earlier version of this list said `sec:nyquist` still wanted a
  Nyquist plot. It already had one — `nyquist_margins`, from Round 3. That item was
...
### ✅ Tier B — complete. All seven landed in Round 5.
