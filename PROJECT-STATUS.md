...
- **23 chapters + 3 appendices, five parts, 197 pages, 29 figures.**
...
Section labels, all 58 of them: `sec:complexrefresher` `sec:rms` `sec:secondorder`
...
`sec:halfpower` `sec:polegeometry` `sec:seriesparallel` `sec:lnetwork` `sec:conductionangle` `sec:switching` `sec:pushpull` `sec:sparams` `sec:vna` `sec:feedbackz` `sec:antennaeff`.
...
so nothing can be silently dropped. Current score: **34 done, 23 open**, with the open
